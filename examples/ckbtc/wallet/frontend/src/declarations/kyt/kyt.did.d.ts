...
export interface Alert {
    service: [] | [string];
    level: { Low: null } | { High: null } | { Medium: null } | { Severe: null };
    category: [] | [string];
    exposure_type: { Indirect: null } | { Direct: null };
}
...
export type LifecycleArg = { UpgradeArg: UpgradeArg } | { InitArg: InitArg };
export type Mode = { RejectAll: null } | { Normal: null } | { AcceptAll: null };
export interface Response {
    provider: Principal;
    alerts: Array<Alert>;
    external_id: string;
}
export interface SetApiKeyArg {
    api_key: string;
}
...
export interface _SERVICE {
    fetch_utxo_alerts: ActorMethod<
        [DepositRequest],
        { Ok: Response } | { Err: FetchUtxoAlertsError }
    >;
    fetch_withdrawal_alerts: ActorMethod<
        [WithdrawalAttempt],
        { Ok: Response } | { Err: FetchWithdrawalAlertsError }
    >;
